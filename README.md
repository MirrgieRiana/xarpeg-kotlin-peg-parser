...
- Operator-driven grammar definitions use Xarpeg's PEG-style operators (`+` for choice, `*` for sequence, unary `-` to drop matches), so rules stay short and scannable without overloading `+`/`*` as quantifiers.
